...
BEGIN
  FOR i IN 0..meses_adelante LOOP
    inicio_mes  := date_trunc('month', NOW())::date + (i || ' months')::interval;
    fin_mes     := inicio_mes + INTERVAL '1 month';
    nombre_part := 'consumption_' || to_char(inicio_mes, 'YYYY_MM');
    ...
    IF NOT EXISTS (
      ...
      EXECUTE format(
        'CREATE TABLE consumption.%I PARTITION OF consumption.consumption
           FOR VALUES FROM (%L) TO (%L)',
        nombre_part, inicio_mes, fin_mes
      );
    END IF;
  END LOOP;
END;
